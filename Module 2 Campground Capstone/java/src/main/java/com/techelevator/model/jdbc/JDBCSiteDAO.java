...
public class JDBCSiteDAO implements SiteDAO {
	...
	public List<Site> getAvailableSitesByParkId(Long parkId, LocalDate startDate, LocalDate endDate) {
		List<Site> siteList = new ArrayList<>();
		String sqlSite = "SELECT DISTINCT * FROM site WHERE site_id NOT IN (SELECT site.site_id "
				+ "FROM reservation INNER JOIN site ON site.site_id = reservation.site_id INNER JOIN campground "
				+ "ON campground.campground_id = site.campground_id WHERE "
				+ "((? BETWEEN reservation.from_date AND reservation.to_date) OR (? BETWEEN reservation.from_date AND reservation.to_date) "
				+ "OR (reservation.from_date BETWEEN ? AND ?) OR (reservation.to_date BETWEEN ? AND ?))) ORDER BY site.campground_id";
		SqlRowSet results = jdbcTemplate.queryForRowSet(sqlSite, startDate, endDate, startDate, endDate, startDate, endDate);
		while (results.next()) {
			Site site = mapRowToSite(results);
			siteList.add(site);
		}
